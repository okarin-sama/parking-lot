...
ParkingSystem.prototype.mapParkingSlot = function (slotStructure = []) {
     ...
     */
    let entryPointsAllocated = slotStructure.every(x => x.length === this.entryPoint.length)
    ...
    if (vehicleHasSize && this.sizes.includes(vehicleInfo.size) && validSlotsAndParkingSize) {
        ...
        if (this.parkingSlot.length === (this.vacantSlot.length + this.occupiedSlot.length)) {
            ...
            let entryPayload = {
                ...
                vehicleInfo,
                time: vehicleInfo && vehicleInfo.time && moment(vehicleInfo.time) || moment().format('YYYY-MM-DD HH:mm:ss') //ignore past time as for testing purposes
            }
            ...
            if (vacantInfo && vacantInfo.vehicleInfo && vacantInfo.vehicleInfo.plate !== vehiclePlate) delete entryPayload.charge
